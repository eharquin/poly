 ...
 * Avec `onSelect`, chaque case se tape ; `activeString` limite les taps à une
 * corde et estompe les autres.
 */
...
export default function FretboardDiagram({ marks = [], label, onSelect = null, activeString = null }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className={`fretboard ${interactive ? 'interactive' : ''}`} role={interactive ? 'group' : 'img'} aria-label={label ?? 'Manche'}>
      ...
      {Array.from({ length: 6 }, (_, s) => (
        <line key={`s${s}`} x1={X0 - OPEN_W} y1={y(s)} x2={X0 + CW * FRETS} y2={y(s)} className={`fret-string ${activeString !== null && activeString !== s ? 'dim' : ''}`} style={{ strokeWidth: 0.8 + (5 - s) * 0.25 }} />
      ))}
      {/* numéros de case */}
      {INLAYS.map((f) => (
      ...
      {/* positions pointées */}
      {marks.map((m) => (
        <circle key={`m${m.string}-${m.fret}`} cx={cx(m.fret)} cy={y(m.string)} r={7} className="fret-dot" />
      ))}
      {/* zones cliquables */}
      {interactive &&
        Array.from({ length: 6 }, (_, s) =>
          activeString !== null && activeString !== s
            ? null
