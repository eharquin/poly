...
export const FRETBOARD_NOTE_CARDS = STANDARD_TUNING.flatMap((open, string) =>
  Array.from({ length: FRETS + 1 }, (_, fret) => {
    ...
    return { id: `s${string}f${fret}`, string, fret, pc, name: pitchClassName(pc), stringLabel: STRING_LABELS[string] }
  }),
)
...
export const FRETBOARD_FIND_CARDS = STANDARD_TUNING.flatMap((open, string) =>
  Array.from({ length: 12 }, (_, pc) => {
    ...
    return { id: `s${string}:${pc}`, string, pc, name: pitchClassName(pc), stringLabel: STRING_LABELS[string], frets }
  }),
)
